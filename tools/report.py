...
def performance(tr:TraceReader, html:HTMLWriter):
    ...
    write_duration, read_duration = 0, 0
    for rank in range(tr.procs):
        write_duration = max(write_df[write_df['rank'] == rank]['duration'].sum(), write_duration)
...
def function_statistics(tr: TraceReader, html:HTMLWriter):
    ...
    # I/O access sizes bar chart
    xs, ys = [[], []], [[], []]
    df = tr.get_posix_io()
    write_df = df[(df['func'].str.contains("write"))]
    for count in sorted(set(write_df['count'])):
        xs[0].append(count)
        ys[0].append(write_df[write_df['count']==count].shape[0])
    read_df = df[(df['func'].str.contains("read"))]
    for count in sorted(set(read_df['count'])):
        xs[1].append(count)
        ys[1].append(read_df[read_df['count']==count].shape[0])
